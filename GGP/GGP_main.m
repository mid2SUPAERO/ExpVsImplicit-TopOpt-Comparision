...
ncx=3; % number of components in the x direction
ncy=10; % number of components in the y direction
...
folder_name=['Optimization_history_',BC,'_',p.method,'_Volfrac_',vf,'_nelx_',num2str(nelx),...
    '_nely_',num2str(nely),'_ncx&y_',num2str(ncx),'_R_',rs,'_Ngp_',num2str(Ngp),'_SC_',stopping_criteria];
...
%%	  Uncomment to obtain box and cross as starting point
...
%         0;pi/2;0;atan2(nely,nelx);pi/2];
    case 'RIB'
        ...
        Tc = [zeros(length(Xc)/2,1);pi/2*ones(length(Xc)/2,1)];
    otherwise
        Xc=repmat(xx(:),2,1); %component center X
        Yc=repmat(yy(:),2,1); %component center Y
        Lc=2*sqrt((nelx/(ncx+2))^2+(nely/(ncy+2))^2)*ones(size(Xc)); %component length L
...
%             dc_dX(1,1) = dc_dX(1,1) + dc_df*dF_dX;
    ...
    switch BC
        ...
        otherwise
            fval=[(v-volfrac)/volfrac]*100; %START HERE
    ...
    if or(mod(outit,plot_rate) == 0,outit == 1)
        ...
        patchplot2 = patch('Vertices',[Xx,Yy],'Faces',edofMat(:,[2,4,6,8])/2,'FaceVertexCData',(1-xPhys(:))*[1 1 1],...
            'FaceColor','flat','EdgeColor','none'); axis equal; axis off;
        hold on
