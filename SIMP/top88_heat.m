...
edofVec = reshape(nodenrs(1:end-1,1:end-1)+1,n,1);
...
mult = 1 - numel(emptyelts)/n;       % Multiplier for volfrac in case of emptyelts
alldofs = [1:ndof]';
...
while stop_cond
  ...
  %% FE-ANALYSIS
  sK = reshape(KE(:)*(Emin+(E0-Emin)*xPhys(:)'.^penal),16*n,1);
